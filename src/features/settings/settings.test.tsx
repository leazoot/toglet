...
// The notification page and row have their own suite (`notify.test.tsx`).
vi.mock("../notify/NotifySection", () => ({ NotifySection: () => null }));
vi.mock("../notify/NotifyRow", () => ({ NotifyRow: () => null }));
...
function stored(overrides: Partial<SettingsView> = {}): SettingsView {
  return {
    ...
    theme: "system",
    reduceMotion: false,
    // Pinned rather than `system`, so assertions read English whatever the machine's locale.
    language: "en",
    ...overrides,
  };
}
...
describe("the settings store", () => {
  ...
  it("holds what Rust says is stored, not what was asked for", async () => {
    // Rust corrects an out-of-range value; the stored value is what must show.
  ...
  it("leaves the settings in force showing when a change did not get through", async () => {
    ...
    expect(held.state).toBe("ready");
    if (held.state !== "ready") return;
    // The old values are still the ones in force.
  ...
  it("never uses the motion setting to take away a system preference", async () => {
    // Off means "follow the system", so the attribute is removed, not inverted.
...
describe("the settings sheet", () => {
  ...
  // The sheet's rows read their own pages' state; answer those reads here.
  beforeEach(() => {
    invoke.mockReset();
    invoke.mockImplementation((command: string) => {
      if (command === "read_notify_channels") {
        return Promise.resolve({ channels: [], maxChannels: 8 });
      }
      if (command === "read_remote") {
  ...
  function sheet(
    ...
    return { onChange, onRemove, onPinned };
  }
  ...
  // Sub-page fields start empty, so closing on pointer drift would lose a half-typed secret.
  it("holds the panel open while a sub-page is showing, and lets go on the way back", async () => {
    const { onPinned } = sheet();
    expect(onPinned).toHaveBeenLastCalledWith(false);

    const row = await screen.findByTestId("remote-row");
    fireEvent.click(within(row).getByRole("button"));

    await screen.findByTestId("remote-section");
    expect(onPinned).toHaveBeenLastCalledWith(true);
  ...
  it("offers two languages, each named in itself", () => {
    sheet();
  ...
  it("marks the language a stored `system` currently resolves to", () => {
    // A fresh install holds `system`; one button must still be marked.
  ...
  it("offers the ring form beside the edge, and asks for it alone", () => {
    const { onChange } = sheet();

    screen.getByRole("radio", { name: "Rings" }).click();

    expect(onChange).toHaveBeenCalledWith({ dockShape: "ring" });
  });
  ...
  it("says the settings could not be read rather than showing defaults as if they were stored", () => {
    render(
      <SettingsSheet
        settings={{ state: "failed", failure: { command: "read_settings", error: null } }}
        saving={false}
        onChange={() => undefined}
        onClose={() => undefined}
