...
describe("the collapsed bar", () => {
  ...
  it("offers to add an account when none has been added", () => {
    // Until the first account exists the bar is the whole interface, so it needs a control.
  ...
  it("offers to pick an account, not to add one, when accounts exist but none is current", () => {
    // That state's next step is to switch to one of the rows.
  ...
  it("draws the arc from the ring formula", () => {
    const { container } = bar();
  ...
  it("tells a reading that failed apart from one the server did not return", () => {
    bar({ quota: { state: "failed", failure: { command: "refresh_quota", error: null } } });
