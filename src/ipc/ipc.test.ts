...
describe("the IPC boundary", () => {
  ...
  it("keeps the code Rust reported, and nothing else it sent", async () => {
    // The code is what lets a screen say which step failed. A detail is not: it is redacted but
    // still a message, and messages have held paths. Two fields are copied out by name, so
    // anything else the rejection carries stays behind.
    invoke.mockRejectedValue({
      code: "app_server_crashed",
      phase: "read_quota",
      retryable: true,
      action: "retry",
      detail: "C:/Users/somebody/.codex",
    });
    ...
    expect(result).toStrictEqual({
      ok: false,
      failure: {
        command: "refresh_quota",
        error: { code: "app_server_crashed", retryable: true },
      },
    });
    expect(JSON.stringify(result)).not.toContain("Users");
  });

  it("reports no error at all when the rejection is not one of Rust's", async () => {
    // A bridge that is not there rejects with whatever the runtime felt like. Half of a shape
    // is not a code the interface may quote at the user.
    invoke.mockRejectedValue({ code: "app_server_crashed" });

    const result = await refreshQuota("acct-1", 0);

    expect(result).toStrictEqual({ ok: false, failure: { command: "refresh_quota", error: null } });
  });
  ...
  it("reports a failed read as failed rather than as an absent value", async () => {
    ...
    expect(result).toStrictEqual({
      ok: false,
      failure: { command: "startup_recovery", error: null },
    });
  });
