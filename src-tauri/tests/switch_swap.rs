...
//!
//! The fake server answers `account/read` from a scenario file rather than the credentials on
//! disk, so a verification can disagree with what was just written.
...
fn a_switch_changes_nothing_in_the_codex_home_but_the_authentication() {
    // A switch touches one file; everything else in the Codex home must be byte-identical.
    let world = World::new(None);
...
fn an_identity_that_does_not_match_the_target_rolls_back_rather_than_reporting_success() {
    // The home reports somebody else no matter what was written.
...
fn a_switch_that_is_interrupted_leaves_a_journal_that_says_what_to_do() {
    // Stands in for the process being killed: recovery finds what the last completed phase wrote.
    let world = World::new(None);
...
fn the_journal_written_during_a_real_switch_carries_no_credential_material() {
    ...
    // Nothing written outside the Codex home may carry credential material; `auth.json` inside
    // it is the credentials by definition.
...
fn a_kill_before_the_replacement_completed_is_rolled_back() {
    // Recovery restores the backup rather than working out how far the interrupted write got.
    let world = World::new(None);
...
fn a_kill_after_an_unverified_replacement_is_verified_before_anything_is_concluded() {
    // The home reports the target, so the switch landed and is completed.
    let world = World::new(None);
...
fn a_recovery_that_cannot_restore_tells_the_user_where_the_copy_is() {
    // The backup is gone, so recovery says it cannot restore rather than reporting a clean state.
    let world = World::new(None);
...
fn a_step_that_never_finished_is_never_announced() {
    // Only the two steps that actually finished are reported.
    let world = World::new(None);
    let observer = RecordingObserver::default();
...
// Signing out: a switch with no target, verified the same way.
...
impl World {
    fn sign_out(&self) -> std::result::Result<(), SignOutFailed> {
        ...
        sign_out
            ...
            // A source scan in `storage::settings` restricts where this token may be issued.
...
fn a_sign_out_the_server_does_not_confirm_is_rolled_back() {
    // The default scenario keeps reporting an account after the file is removed; the removal
...
fn a_kill_during_a_sign_out_restores_the_authentication_at_the_next_start() {
    // A sign-out journal names no target, so recovery always restores.
    let world = World::new(Some("signed_out"));
    let backup = world.home.path().join("auth.json.toglet-switch-op-crash");
    std::fs::write(&backup, auth_json(ACCOUNT_ID)).expect("written");
