...
//!
//! Not covered: a person completing OAuth in a browser, or adding a second real account.
...
fn stored_credentials_can_be_decrypted_and_used_again() {
    ...
    // A stored snapshot has to be usable, not merely present.
...
fn credentials_that_identify_nobody_are_refused_rather_than_stored() {
    // Matched rather than `expect_err`, which needs `Debug` on a type owning decrypted material.
...
fn the_account_the_default_home_already_holds_can_be_recorded_as_the_active_one() {
    // Without this, `activeAccountId` stays null after an import and the pre-checks refuse the
    // first switch.
    let verified = verify_with("normal", auth_json("id-1", "chatgpt")).expect("verified");
