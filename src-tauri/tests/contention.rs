//! Lock contention and a file held open by another process: the cases that only appear when two
...
fn only_one_of_many_threads_racing_for_the_switch_lock_gets_it() {
    // Two barriers rather than a sleep: the first makes every thread attempt at once, the second
    // stops the winner releasing until all have tried. Without them the threads take turns.
...
fn a_switch_waits_for_a_credential_write_back_instead_of_reading_a_snapshot_being_replaced() {
    // The write-back finishes and the next credential reader waits for it. Channels, not sleeps,
    // set the order; the waiting thread can only say "about to block", but the assertion holds
    // because the write-back records its end before releasing the lock.
...
/// A target file another process holds open for reading only, so the copy succeeds and the
/// switch reaches the replacement step.
#[cfg(windows)]
#[test]
fn a_replacement_that_the_operating_system_refuses_reports_where_the_backup_is() {
    ...
    // Taken after the checks, as if another program had just opened the file.
...
fn a_failed_pre_check_names_the_step_and_a_stable_code() {
    // Every pre-check failure carries the check that stopped it and a stable code.
    let world = World::new();
    let binary = fake_binary(PHASE);
    let probe = NoClients;
    let preflight = Preflight {
