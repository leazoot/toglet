//! Refresh behaviour shown by running it against the fake server: one app server at a time,
//! nothing left behind, and the default Codex home never touched.
...
fn walking_the_queue_runs_one_app_server_at_a_time() {
    ...
    // Disjoint intervals are the observable form of "at most one subprocess at a time".
