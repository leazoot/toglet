...
//!
//! Pins down Toglet's policy: when it refuses, what it backs up, and that an ineffective write is
...
/// A session presenting [`ServerHome::Default`], which the config manager requires, over a
/// throwaway directory the returned [`IsolatedHome`] deletes.
...
fn a_configuration_that_cannot_be_parsed_stops_the_operation_and_names_the_file() {
    // A broken `config.toml` answers `-32603` with no `data`, so only the method distinguishes it;
    // reporting an incompatible runtime would send the user to update Codex over a typo.
...
fn what_managing_recorded_survives_a_restart_and_drives_the_restore() {
    // The record must outlive the process, so it goes through the real metadata document.
    let (home, mut session) = default_home_session("config_other_value");
