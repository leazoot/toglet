//! The seven pre-checks, each failing on its own and reporting which step stopped the switch.
...
/// A probe that answers whatever the test says, so every client-check branch is reachable.
...
fn check_six_a_home_that_cannot_be_written_stops_the_switch() {
    let world = World::new();
    let probe = nothing_running();
    // A path that is not a directory, so creating the probe file inside it fails.
...
fn check_seven_a_sign_in_that_happened_outside_toglet_stops_the_switch() {
    // The default home holds an unknown account; replacing it would silently discard that session.
    let world = World::new();
    let probe = nothing_running();
    std::fs::write(
...
fn the_verdict_helper_and_the_probe_agree_about_an_empty_machine() {
    // The pre-check uses `verdict`, and this fake has the shape the real probe returns.
