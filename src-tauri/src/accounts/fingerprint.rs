...
//!
//! The fingerprint only recognises already-added accounts and must never be logged. Post-switch
//! verification compares e-mail addresses instead (in `switching`), because `account/read`
//! returns no stable id.

use sha2::{Digest, Sha256};

/// Domain separator, not a secret: keeps this digest distinct from other digests of the same input.
const DOMAIN: &[u8] = b"toglet.account-fingerprint.v1";

/// SHA-256 because the input is a UUID from a small structured space, which a fast hash would
/// make worth inverting.
...
///
/// Only lowercase and trim: addresses come from the app server, and Gmail-style dot or alias
/// folding would merge distinct mailboxes on most other providers.
...
fn digest(kind: &[u8], value: &[u8]) -> String {
    ...
    hasher.update(kind);
    hasher.update(value);
    // Hex rather than base64: no `+` or `/` to confuse a file name.
    hasher
        .finalize()
        .iter()
...
pub enum DuplicateCheck {
    New,
    /// The caller offers to refresh the existing account's credentials instead of adding a
    /// second profile.
    AlreadyPresent {
        existing_id: String,
    },
...
/// `existing` pairs each known internal id with its fingerprint.
...
mod tests {
    ...
    fn dots_and_aliases_are_left_alone_because_they_are_different_mailboxes() {
        // Gmail's dot rule applied everywhere would merge real accounts on most other providers.
    ...
    fn the_two_derivations_never_collide_for_the_same_text() {
        // Domain separation: an account id that looks like an address must not match it.
