...
/// Hard limit on stored accounts; a product choice, not a technical ceiling.
pub const MAX_ACCOUNTS: usize = 12;

/// Adds a profile, refusing to exceed the limit.
///
/// A duplicate is not an error: the existing id is returned so the caller can offer a refresh.
...
/// Refuses the active account, which would leave Codex signed in as an account Toglet no longer
/// knows; the caller switches away or signs out first.
