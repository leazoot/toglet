...
//! Holds no credential material; a test also checks the written bytes. Only one exists at a
//! time, and a switch that finishes deletes it along with the backup.
...
pub struct SwitchJournal {
    ...
    /// `None` for a sign-out: there is no target to confirm, so recovery restores the copy.
    pub to_account_id: Option<String>,
    pub phase: SwitchPhase,
    /// Where the previous authentication was copied to. An absolute path; never logged or sent to
    /// the frontend.
...
impl SwitchJournal {
    ...
    /// Removes the journal and the backup it points at, once the outcome is verified.
    pub fn finish(self, directory: &Path) -> Result<()> {
        // Journal first: a leftover backup is inert, but a journal pointing at a deleted backup
        // would send recovery looking for a missing file.
        remove(&directory.join(JOURNAL_FILE), Phase::Storage)?;
        drop(std::fs::remove_file(&self.backup_path));
        Ok(())
    }

    /// Reads the journal, if a switch was interrupted. An unparseable file is an error rather than
    /// "nothing in flight", because that decides whether the previous authentication is restored.
...
pub enum RecoveryPlan {
    ...
    RestoreBackup,
    /// The replacement landed but was never verified: re-check, then finish or roll back.
    ReVerify,
}

impl RecoveryPlan {
    /// `BackedUp` restores rather than re-verifying: the replacement may have been interrupted
    /// half-way.
