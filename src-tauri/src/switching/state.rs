//! How far a switch has actually got: Check → Switch → Verify → Ready.
//! No clock is read and a step can only follow the previous one, so progress cannot advance on
//! a timer while the work is stuck.
...
/// Told when a step really finished. Called only where `SwitchProgress` records the step, so it
/// cannot be driven by a timer, and it cannot influence the switch.
...
impl SwitchProgress {
    ...
    /// Records that `step` finished. Anything but the next step is a caller bug, hence `Internal`
    /// and not retryable.
