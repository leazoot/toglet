//! Reading and writing the metadata document: a single JSON file replaced atomically.
...
pub enum LoadOutcome {
    ...
    Created,
    /// The file could not be used and was replaced with a fresh document. Credentials in the
    /// platform store are untouched.
...
impl MetadataStore {
    /// `directory` must already exist and be private.
    ...
    /// Loads the document, rebuilding it if it cannot be used; a damaged file never stops startup.
    pub fn load(&self) -> (MetadataDocument, LoadOutcome) {
        let Ok(text) = std::fs::read_to_string(&self.path) else {
            return (MetadataDocument::default(), LoadOutcome::Created);
        };

        // Read the version first: a newer build's document usually fails a full parse, which
        // would otherwise be reported as unreadable.
...
mod tests {
    ...
    fn a_file_from_a_newer_build_is_refused_rather_than_reinterpreted() {
        let (_directory, store) = store();
        // Also unparseable as a whole, so the version has to be readable on its own.
        std::fs::write(
            store.path(),
    ...
    fn an_interrupted_save_leaves_the_previous_document_intact() {
        ...
        // Crashing mid-write is covered by `atomic_write`'s tests; here a second save must fully
        // replace the first.
    ...
    fn what_lands_on_disk_carries_no_credential_material() {
        ...
        // Checked on the actual file rather than trusting the struct definition.
