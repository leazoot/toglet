...
//! A refresh inside a throwaway home would otherwise be deleted with it, leaving a stale (or, with
//! rotating refresh tokens, dead) snapshot. Detected by byte comparison.
...
/// Serialises credential write-backs against a switch reading them.
///
/// A write-back in progress finishes and a switch waits: discarding a refreshed token could leave
/// the account unusable if refresh tokens rotate (not confirmed for Codex), while waiting costs
/// milliseconds. `switching` must hold this around reading the snapshot, replacing and verifying.
...
impl CredentialLock {
    ...
    pub fn acquire(&self) -> MutexGuard<'_, ()> {
        // Poisoning is harmless: the guarded data is `()`.
        self.inner
            .lock()
...
/// Stores the credentials in `home` if they differ from `original`. On failure the stored
...
mod tests {
    ...
    fn a_failed_write_back_leaves_the_old_snapshot_intact() {
        ...
        // The store refuses every operation.
