//! Decrypting one account's credentials into a throwaway Codex home. The `IsolatedHome` guard
//! removes it on every path, but removal is best effort, so the private permissions matter more.
//! Plaintext lives only in a local `Secret`, which clears its buffer on drop.
...
impl CredentialSession {
    /// Decrypts `reference` into a fresh isolated home, removed again on any failure.
    pub fn open(store: &dyn SecretStore, reference: &CredentialRef, phase: Phase) -> Result<Self> {
        ...
        // Permissions are applied on creation, so the file is never briefly readable by others.
...
mod tests {
    ...
    fn a_store_failure_names_no_path_and_leaves_no_credentials() {
        ...
        // Home cleanup on this path is covered by `codex_home::isolated`'s early-return test.
    }
