...
//! Scope is `CurrentUser` (`CRYPTPROTECT_LOCAL_MACHINE` is never passed), so the boundary is other
//! users; code already running as this user is out of scope (see `SECURITY.md`).
...
/// Application-specific entropy mixed into every blob. Not a key (it ships in the binary); it keeps
/// unrelated software running as the same user from decrypting blobs, and foreign blobs fail here.
...
/// Runs one DPAPI call and copies the result out of the OS-allocated buffer, which is then freed.
fn with_blob(
    data: &[u8],
    entropy: &[u8],
    call: impl FnOnce(
    ...
    // The input blobs only borrow `data` and `entropy`; there is nothing to release for them.
    let input = blob(data)?;
