...
//! The DACL is built from scratch with one allow ACE for the token user. `SE_DACL_PROTECTED` is
//! required: without it Windows merges the parent's inheritable ACEs into the new object.
...
pub(super) fn is_private(path: &Path) -> io::Result<bool> {
    ...
    // Exactly one ACE, for the token user; this also rules out `Everyone`, `Users` and
    // `Authenticated Users`.
