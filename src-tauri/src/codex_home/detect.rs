//! First-run environment detection: seven checks, reported separately and never merged.
//! A check that could not run says so rather than passing. The only executable started is the
...
/// How a Codex home is authenticated: the `AuthMode` values the app server protocol defines.
...
enum AuthState {
    ...
    HostManagedTokens,
    /// A mode this build does not know, reported as-is.
    Unknown(String),
}
...
pub fn detect_environment() -> EnvironmentReport {
    ...
    checks.push(match &binary {
        ...
        // Without an executable there is nothing to ask; never reported as passed.
...
/// Starts an app server in a throwaway home and calls `account/read`, which answers locally
/// without an account or the network. Availability is proven by calling, not by version.
...
/// Checks `config.toml` is readable and writable without changing it (append-mode open).
/// A missing file passes: Toglet creates it when needed.
...
/// Reads only `auth_mode` from the default `auth.json`; serde skips the tokens, so they are never
/// materialised.
...
fn operating_system_version() -> Option<String> {
    ...
    // SAFETY: `info` is a correctly sized OSVERSIONINFOW. `RtlGetVersion` is used because
    // `GetVersionExW` reports a capped version without a compatibility manifest.
...
#[cfg(not(windows))]
fn operating_system_version() -> Option<String> {
    // The plist is plain XML, so the one value is read without a parser.
