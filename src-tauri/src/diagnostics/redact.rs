//! Redaction of secrets and quasi-sensitive values, applied when a diagnostic string is built,
//! never as a later filter. A hand-written scanner rather than a regex engine: the patterns are
//! few and anchored, and it keeps dependencies out of a credential-handling binary.
...
/// POSIX roots that mark an absolute path. A bare `/` is not a path start, so prose survives.
...
///
/// Covers, in order: `http`/`https` URLs, OAuth/token parameter values, JWT-shaped tokens,
/// `sk-` API keys, e-mail addresses (removed, not masked) and absolute paths.
pub fn redact(input: &str) -> String {
    ...
    while i < bytes.len() {
        // Before the parameter matcher so a callback URL goes whole: its host can identify the
...
/// An `http`/`https` URL up to the first whitespace, removed whole since any part can disclose.
