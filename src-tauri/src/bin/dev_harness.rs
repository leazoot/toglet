//! Development-only binary, built only with the `dev-harness` feature, for driving the account
//! loop without a user interface. It prints nothing the application itself would not.
...
use toglet_lib::credentials::{CredentialLock, CredentialRef, MemorySecretStore, SecretStore};
use toglet_lib::diagnostics::{ErrorCode, Phase, TogletError, UserAction};
use toglet_lib::notify::{
    CHANNELS_SCHEMA_VERSION, ChannelBook, ChannelConfig, ChannelKind, Connection, Message,
    deliver_all, prepare, store_connection,
};
...
fn main() {
    ...
    if let Err(error) = result {
        // No detail: it can carry an operating-system message, and those carry paths.
        println!(
        ...
        // The redacted detail, printed for a developer and never returned to a caller.
...
fn help() {
  ...
  notify <url>      post one test message to a plain webhook, through the whole outbound path
...
/// Creates the directory, or adopts an existing one only after checking it is private.
...
fn store() -> Result<Box<dyn SecretStore>, TogletError> {
    ...
    #[cfg(target_os = "macos")]
    {
        Ok(Box::new(toglet_lib::credentials::FileSecretStore::new(
            directory,
        )))
    }
    #[cfg(not(any(windows, target_os = "macos")))]
    {
        let _ = directory;
        Err(internal(
            "the harness only wires the Windows and macOS credential stores",
        ))
    }
}
...
/// Signs in to a new account inside a throwaway home; the default Codex home is never touched.
fn login(name: Option<&String>) -> Result<(), TogletError> {
    ...
    if outcome != LoginOutcome::Completed {
        // The sign-in produced no credentials, so there is nothing to store.
        pending.finish()?;
...
fn login_cancel() -> Result<(), TogletError> {
    ...
    pending.cancel()?;
    // A cancellation must be reported as a cancellation, not as a failure.
...
/// Failure injection for manual switch checks, supplied the same way the integration tests do.
...
impl Faults for HarnessFaults {
    fn before(&self, stage: SwitchStage) -> Result<(), TogletError> {
        ...
        if self.fail_at == Some(stage) {
            // The phase follows the stage so the output shows where the switch stopped.
...
fn refresh(id: Option<&String>) -> Result<(), TogletError> {
    ...
    // `atomic_write` applies the permissions at creation, as the quota path does.
...
fn switch(id: Option<&String>, arguments: &[String]) -> Result<(), TogletError> {
    ...
    // The switch duration is measured from after the pre-checks.
        ...
        Err(failed) => {
            match &failed.rollback {
                RollbackReport::Failed { backup } => {
                    // The one place a path is printed: the previous credentials are in that file.
                    println!(
...
fn recovery() -> Result<(), TogletError> {
    ...
    // No expected identity, so recovery rolls back rather than completing on an assumption.
...
fn timestamp() -> String {
    // Seconds since the epoch: the harness has no date formatter.
    format!("{}", now_seconds())
}
...
fn sync(active: Option<&String>) -> Result<(), TogletError> {
    ...
    // Without an active account every sign-in looks external, so one is taken from the document
    // or named.
...
/// The outcome's name only: `ExternalLogin`'s `Debug` form carries an account fingerprint.
...
/// Who the default Codex home is signed in as, read-only through a freshly started app server.
...
/// Sends one message to a plain webhook through the full production path, end to end.
fn notify(url: Option<&String>) -> Result<(), TogletError> {
    let Some(url) = url else {
        return Err(
            TogletError::new(ErrorCode::Internal, Phase::Notify, false, UserAction::None)
                .with_detail("an address is required"),
        );
    };

    let secrets = MemorySecretStore::default();
    let connection = Connection::Webhook { url: url.clone() };
    connection.validate()?;
    store_connection(&secrets, "chan-dev", &connection)?;

    let book = ChannelBook {
        schema_version: CHANNELS_SCHEMA_VERSION,
        channels: vec![ChannelConfig {
            id: "chan-dev".to_owned(),
    ...
    let message = Message::new("Toglet", "A message from the development harness.");
    let outcomes = tauri::async_runtime::block_on(deliver_all(targets, &message, 0));

    for (outcome, _) in outcomes {
        // The host, never the address: the path is where a webhook's token lives.
        println!(
            "channel {}  host {}  sent {}  code {}",
            outcome.channel_id,
            connection.hint(),
            outcome.ok,
            outcome.code.unwrap_or_else(|| "-".to_owned())
        );
    }
    Ok(())
}
