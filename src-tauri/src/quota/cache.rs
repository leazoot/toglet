//! Quota snapshots and staleness.
//!
//! `stale` is derived from `fetched_at` and the clock, never stored, so only a new reading can
//! make a snapshot fresh. A stale snapshot keeps its values.
...
pub struct QuotaSnapshot {
    ...
    /// Unix seconds of the last successful read; only a new reading moves it.
    fetched_at: i64,
...
impl QuotaSnapshot {
    ...
    /// Records a failed attempt; the last values and `fetched_at` are kept.
    ...
    /// How old the reading is, clamped at zero when the clock jumps backwards.
...
mod tests {
    ...
    fn there_is_no_way_to_clear_staleness_without_a_reading() {
        // Only `fresh` and `record_success` move `fetched_at`, and both take a reading.
        let mut snapshot = QuotaSnapshot::fresh("acct-1", quota(2.0), 0);
        snapshot.record_failure("network_unavailable");
