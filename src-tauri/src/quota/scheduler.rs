...
//!
//! Pure functions of state and a clock reading, so long outages can be simulated in tests.
//! This module cannot write the default authentication; a test scans its source to keep it so.
...
/// The backoff stops growing here.
...
impl Backoff {
    ...
    /// Records a failure; saturating, so a long outage cannot overflow into a short delay.
    #[must_use]
    ...
    /// Clears the history so the next failure starts from the base delay.
    #[must_use]
    ...
    pub fn delay(self) -> Duration {
        ...
        // `checked_shl`: a plain shift past the type width would wrap to a tiny delay.
...
pub struct RefreshState {
    ...
    pub backoff: Backoff,
    /// The account Codex is signed in as; it refreshes more often.
    pub is_active: bool,
    /// Accounts that cannot be read are not retried on a timer.
    pub is_refreshable: bool,
}

/// Why a refresh was scheduled.
...
/// Ordered work list for a regular tick, active account first.
///
/// A queue, not parallel jobs: callers walk it one entry at a time so only one app server runs.
pub fn due_now(
    states: &[RefreshState],
    now: i64,
...
fn ordered(
    ...
    // Active first, then oldest first; a stable sort keeps the order predictable.
...
mod tests {
    ...
    fn eight_hours_offline_does_not_produce_high_frequency_retries() {
        ...
        // Unbounded retrying at 60 s would be 480 attempts; the cap settles at four an hour.
        assert!(
            attempts <= 40,
    ...
    /// Refreshing must not be able to touch the default authentication. A source scan, since
    /// the language cannot forbid one module from calling another.
    #[test]
